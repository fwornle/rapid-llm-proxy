...
export class LLMService extends EventEmitter {
  ...
  constructor(config?: LLMServiceConfig) {
    ...
    this.circuitBreaker = new CircuitBreaker(
      this.config.circuitBreaker?.threshold || 3,
      this.config.circuitBreaker?.resetTimeoutMs || 30000,
    );
        ...
        if (request.responseSchema && result.content) {
          try {
            ...
            const retryMessages = [
              ...
              { role: 'user' as const, content: `The response did not match the required schema. Error: ${validationError instanceof Error ? validationError.message : String(validationError)}. Please fix the JSON output.` },
            ];
  ...
  private stripMarkdownFences(raw: string): string {
    ...
    if (!trimmed.startsWith('{') && !trimmed.startsWith('[')) {
      ...
      if (jsonStart >= 0) {
        ...
        if (lastClose > jsonStart) return trimmed.slice(jsonStart, lastClose + 1);
      }
    }
   ...
   */
  private parseJsonWithRepair(raw: string): unknown {
    try {
   ...
   */
  private closeUnclosedBrackets(s: string): string {
    const stack: string[] = [];
    let inString = false;
    let escaped = false;
